...
class AllowBitrixIframe
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Check if request is originating from Bitrix24 embedded context (widgets, OAuth, installation handlers)
        if ($request->is('b24/*') || $request->is('api/b24/*') || $request->is('/') || $request->is('login')) {
            $response->headers->remove('X-Frame-Options');

            // Dynamic tenant domain support for on-premise or cloud Bitrix24 portals
            ...
            // Main administrative portal pages: block framing completely (Anti-Clickjacking)
